...
import { generateResponse, createTaskResponse, availableModels } from './services/ai.js';
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    const allowed = ['text/plain', 'application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'text/csv', 'image/jpeg', 'image/png', 'image/webp'];
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: userId, email: email.toLowerCase(), name: name || 'User' }, config.jwtSecret, { expiresIn: '7d' });
...
app.post('/api/chat', authMiddleware, async (req, res) => {
  ...
  const response = await generateResponse({ prompt: message, userId: req.user.id, conversationId: conversationId || randomUUID(), provider: config.aiProvider });
